package com.beyond.restapi.section04.hateoas;

import com.beyond.restapi.section02.responseentity.ResponseMessage;
...
public class HateoasTestController {
    ...
    /* EntityModel : 데이터(UserDTO)와 링크를 함께 담는 래퍼 클래스
    *  -> Spring HATEOAS 핵심 클래스 */
		List<EntityModel<UserDTO>> userWithRel =
				users.stream().map( // 스트림에서 MAP은 기존 요소를 꺼내서 리턴값을 이용해 새로운 스트림을 만든다
					user -> EntityModel.of(
						...
						.withSelfRel(), // 내 링크 다시 돌려줌 (ex) 물은 셀프입니다!!)
						linkTo(
							methodOn(HateoasTestController.class)
							.findAllUsers()
	...
	public ResponseEntity<ResponseMessage> findUserByNo(@PathVariable int userNo) {
		...
		Map<String, Object> responseMap = new HashMap<>();
		responseMap.put("user", foundUser); // 전체 조회된 User + 링크
		
		return ResponseEntity 
				.ok()
				.headers(headers)
				.body(new ResponseMessage(200, "조회 성공", responseMap)); // 조회 성공으로 리턴하겠다!
	}
}