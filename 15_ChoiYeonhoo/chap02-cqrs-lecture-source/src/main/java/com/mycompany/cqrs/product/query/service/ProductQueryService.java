package com.mycompany.cqrs.product.query.service;


import com.mycompany.cqrs.common.dto.Pagination;
...
import com.mycompany.cqrs.product.query.dto.response.ProductDTO;
...
import com.mycompany.cqrs.product.query.mapper.ProductMapper;
...
@Service
@RequiredArgsConstructor // final이 붙은 필드를 초기화 하는 생성자 추가 어노테이션
public class ProductQueryService {
  ...
  /* 읽기 전용 Transaction을 이용해서 성능 최적화 + 실수로 데이터 변경 방지 */
  @Transactional(readOnly = true)
  public ProductDetailResponse getProduct(Long productCode){

//    ProductDTO product = productMapper.selectProductByCode(productCode); // 이전에 mybatis 배울떄 쓰던 방식
    ...
    * */
    ...
    // 조회 결과가 있으면 ProductDTO 객체 반환
    // 조회 결과가 없으면 예외 발생
    ProductDTO product = Optional.ofNullable(
        productMapper.selectProductByCode(productCode)).orElseThrow(()-> new BusinessException(ErrorCode.PRODUCT_NOT_FOUND));
