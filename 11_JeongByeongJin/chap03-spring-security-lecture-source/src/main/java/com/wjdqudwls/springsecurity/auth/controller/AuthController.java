package com.wjdqudwls.springsecurity.auth.controller;

import com.wjdqudwls.springsecurity.auth.dto.LoginRequest;
import com.wjdqudwls.springsecurity.auth.dto.TokenResponse;
import com.wjdqudwls.springsecurity.auth.service.AuthService;
import com.wjdqudwls.springsecurity.common.ApiResponse;
...
public class AuthController {
  ...
  public ResponseEntity<ApiResponse<TokenResponse>> refreshToken(
    ...
    // 쿠키에 refresh token 이 없을 경우
    if (refreshToken == null) {
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    }

    /* refreshToken 이 문제가 없다면
     * access/refresh token 발급됨 */
